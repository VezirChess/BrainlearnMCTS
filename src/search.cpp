...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning, bool mcts) {
	...
	Move expTTMove = MOVE_NONE;
    Depth extension, newDepth;
    Value bestValue, expTTValue, value, ttValue, eval;
    ...
    {
      expNode node = getNodeFromHT(posKey,HashTableType::global);
      if (node!=nullptr)
      {
		  ...
		  if (expTTValue >= beta)
            {
                if (!pos.capture_or_promotion(expTTMove))
                    update_stats(pos, ss, expTTMove, nullptr, 0, stat_bonus(depth));
                else
            ...
            // Penalty for a quiet expTTMove that fails low
